...
function tweakCrud(crud : DynamicOutputPropertyDef<"crud">) : DynamicOutputPropertyDef<"crud"> {
  debug(`... tweaking your CRUD!`)
  // Actually we aren't changing a thing (for now). But we could

  const constructorParams : Parameters<typeof dynamicOutputProperty>[0] = (crud as any).config
...
`,
    ...
    factory({ typeDef, typeName, stage, builder }) {
      ...
      const retval = constructorParams.factory({ typeDef, typeName, stage, builder })
      return retval
    }
  })
}
