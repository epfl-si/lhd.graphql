...
import {EXPIRED_DISPENSATION} from "../utils/Email/EmailTemplates";
...
async function expireAndNotifyDispensations () {
	...
	for (const disp of expiredDisps) {
		await prisma.$transaction(async (tx) => {
			...
			await sendEmailForDispensation(cronUser.userFullName, cronUser.userEmail, updated, EXPIRED_DISPENSATION);
		},{
